...
namespace Informa.Library.Salesforce.V2.ProductPreferences
{
    public class SalesforceGetUserProductPreferencesQueryFactory : ISalesforceGetUserProductPreferencesQueryFactory
    {
        ...
        public string Create(string userName, string publicationCode, ProductPreferenceType type)
        {
            ...
            {
                query = "SELECT+Id, Username__c, Product_Vertical__c, Value1__c, Value2__c, Value3__c, Value4__c, Value5__c, Value6__c+from+Product_Preference__c+where+UserName__c='" + userName + "' and Type__c='" + Bookmark + "' and Value1__c='" + publicationCode + "'";
            }
            else if (type == ProductPreferenceType.PersonalPreferences)
            {
                query = "SELECT+Id, Username__c, Product_Vertical__c, Value1__c, Value2__c, Value3__c, Value4__c, Value5__c, Value6__c+from+Product_Preference__c+where+UserName__c='" + userName + "' and Type__c='" + ContentPersonalization + "' and Value1__c='" + publicationCode + "'";
            }
