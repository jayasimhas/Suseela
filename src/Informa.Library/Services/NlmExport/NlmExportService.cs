    ...
    public class NlmExportService : INlmExportService
    {
        ...
        public ExportResult ExportNlm(ArticleItem article, ExportType exportType, PublicationType? type = null)
        {
            try
            {
                ...
                if (validationResult.ValidationSuccessful)
                {
                    ...
                    var articleItem = getItemFromArticle(article);
                    if (articleItem == null)
                        return new ExportResult { ExportSuccessful = false };

                    Directory.CreateDirectory(exportFolder);


                    var fileName = GetFilenamePrefix(article, articleItem) + ".xml";
        ...
        public bool DeleteNlm(ArticleItem article)
        {
            try
            {
                ...
                var fileName = GetFilenamePrefix(article, articleItem) + "_del.xml";
                Directory.CreateDirectory(exportFolder);
        ...
        private string GetFilenamePrefix(ArticleItem article, Item articleItem)
        {
        ...
        private Item getItemFromArticle(ArticleItem article)
        {
            var item = new SitecoreService(Constants.MasterDb).GetItem<Item>(article._Id);

            if (item == null)
                item = new SitecoreService(Constants.WebDb).GetItem<Item>(article._Id);
