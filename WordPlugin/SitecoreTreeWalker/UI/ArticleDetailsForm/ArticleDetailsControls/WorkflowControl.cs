    ...
    public partial class WorkflowControl : ArticleDetailsPageUserControl
    {
        ...
        protected void SetNotificationOptions()
        {
            ...
            else if (uxWorkflowActions.SelectedValue is string)
            {
                ...
                {
                    foreach (var item in _ArticleStruct.Authors)
                    {
                        if (command.GlobalNotifyList.Exists(e => e.ID == item.ID) == false)
                            command.GlobalNotifyList.Add(item);
                    }
                }

                if (command != null && command.GlobalNotifyList != null)
                {
                    uxNotifyList.ResetUnremovableStaff(command.GlobalNotifyList.ToList());
                }
            }
        }
