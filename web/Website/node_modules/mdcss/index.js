...
var isDocumentation = /^\s*-{3,}\n((?:[ \t]*[A-z][\w-]*[ \t]*:[ \t]*[\w-][^\n]*\n*)*)(?:[ \t]*-{3,})?/;
var isFrontMatter = /([A-z][\w-]*)[ \t]*:[ \t]*([\w-][^\n]*)/g;
...
module.exports = require('postcss').plugin('mdcss', function (opts) {
	...
	return function (css, result) {
		...
		css.walkComments(function (comment) {
			...
			if (isDocumentation.test(comment.text)) {
				
				var doc = {};

				// Filter and parse documentation front-matter
				doc.content = marked(comment.text.replace(isDocumentation, function (isDoc0, metas) {
					// push meta to documentation
					//console.log(isDoc0);
					if (metas) metas.replace(isFrontMatter, function (isMeta0, name, value) {
						doc[name] = value.trim();
					});
