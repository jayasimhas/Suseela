		...
		private ArticleItem SaveArticleDetails(ArticleItem article, ArticleStruct articleStruct, bool saveDocumentSpecificData, bool addVersion, bool shouldNotify = true)
		{
			...
			try
			{
				...
				if (addVersion)
				{
					using (new EditContext(articleItem))
					{
						...
						newVersion = articleItem.Versions.AddVersion();
					}
				}
				else
				{
					newVersion = articleItem;
				}
				//newVersion = article;
			}
			catch (Exception ex)
			...
			try
			{
				...
				using (new SecurityDisabler())
				{
					using (new EditContext(newVersion))
					{
						if (articleStruct.CommandID != Guid.Empty)
						{
							...
							_articleUtil.ExecuteCommandAndGetWorkflowState(newVersion, articleStruct.CommandID.ToString());
				...
				using (new SecurityDisabler())
				{
					...
					{
						//TODO: comment plz
						newVersion.NotificationTransientField.ShouldSend.Checked = false;
					}
					if (loggedIn)
					{
						_sitecoreMasterService.GetItem<Item>(newVersion.ID.Guid).Locking.Lock();
					}
				}
			}
